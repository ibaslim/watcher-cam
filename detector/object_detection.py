...
def _category(label: str) -> str | None:
    ...
    if lowered == "person":
        return "person"
    if lowered in {"car", "truck", "bus", "van", "motorbike", "motorcycle", "bicycle", "vehicle"}:
        return "vehicle"
...
def detect_objects(image_bgr: np.ndarray) -> list[ObjectDetection]:
    settings = get_settings()
    model = get_model()
    allowed_tokens = set(settings.yolo_class_list)
    names = model.names
    class_ids = [
        class_id
        for class_id, name in names.items()
        if (_category(name) in allowed_tokens or name.lower() in allowed_tokens)
        and _category(name) is not None
    ]

    if not class_ids:
        log.warning("none of YOLO_CLASSES matched model labels: %s", sorted(allowed_tokens))
        return []
