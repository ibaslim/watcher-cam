...
Runs two things concurrently:
  ...
  1. An HTTP server used for health and worker-refresh requests.
  2. A camera-worker supervisor that pulls the current camera list from the
...
async def main() -> None:
    # Warm AI models before we start workers / accept requests.
    ...
    log.info("detector running: http + camera supervisor")
